...
#include "TransitionNNet.h"


TransitionNNetMemoryManager::TransitionNNetMemoryManager(const int threadNum, const int longestLen, const int batchSize, const int num_in, const int num_hidden, const int num_out, Model<XPU> *modelParasPtr) : m_nThread(threadNum) {
...
TransitionNNetMemoryManager::~TransitionNNetMemoryManager() {
    for (int i = 0; i < m_nThread; i++) {
...
std::vector<NNet<XPU> *> TransitionNNetMemoryManager::getNetPtrVec(const int threadId) {
 ...
 */
void TransitionNNet::updateTNNetParas(Model<XPU> *cumulatedGrads, BeamDecoder &decoder, int &correctSize, double &loss) {
...
void TransitionNNet::updateTNNetParas(Model<XPU> *cumulatedGrads, BatchBeamDecoder &batchDecoder) {
