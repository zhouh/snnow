...
struct NNetPara{
  ...
  NNetPara(int batch_size, int num_in, int num_hidden, int num_out) : rnd(0) {
    ...
    eg2Wi2h.Resize(Shape2(num_in, num_hidden));  
    eg2Wh2o.Resize(Shape2(num_hidden, num_out)); 
    eg2Hbias.Resize(Shape1(num_hidden)); 
    eg2Hbias = 1.0;
    eg2Wi2h = 1.0;
    eg2Wh2o = 1.0;
...
struct UpdateGrads{
    ...
    UpdateGrads(Stream<xpu> *stream, int num_in, int num_hidden, int num_out){
        ...
        cg_Wi2h.Resize(Shape2(num_in, num_hidden));  
        cg_Wh2o.Resize(Shape2(num_hidden, num_out)); 
        cg_hbias.Resize(Shape1(num_hidden)); 
    }
};
...
/*    copytensor.Resize(nhidden.shape_);*/
 ...
 static void UpdateCumulateGrads(UpdateGrads<xpu>& cgrads, NNetPara<xpu>* paras){
    ...
    paras->Wi2h -= CConfig::fBPRate * ( cgrads.cg_Wi2h / ( paras->eg2Wi2h + CConfig::fAdaEps ) );
    paras->Wh2o -= CConfig::fBPRate * ( cgrads.cg_Wh2o / ( paras->eg2Wh2o + CConfig::fAdaEps ) );
    paras->hbias -= CConfig::fBPRate * ( cgrads.cg_hbias / ( paras->eg2Hbias + CConfig::fAdaEps ) );
