...
int WordDictionary::element2Idx(const std::string &s) {
    if (isNumber(s)) {
        return numberIdx;
    }
...
// LabelDictionary
int LabelDictionary::element2Idx(const std::string &s) {
    auto it = m_mElement2Idx.find(s);
...
int CapitalDictionary::element2Idx(const std::string &s) {
    bool isNoncap = true;
    bool isAllcap = true;
